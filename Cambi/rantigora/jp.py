...
import pandas as pd
from 转换 import convSim, convProto

path = 'Cambi/rantigora/'
# 账号
...
if last != content:
    ...
    sim = dat['Lat'].map(lambda x: convSim(x), na_action='ignore')
    ...
    for line in content.split('\n'):
        word = line.split(',')
        cards += card.format(sim = convSim(word[2]), old2 = convProto(word[1]), lat = word[2], han = word[0])
    ...
    sp.run('git add Cambi/rantigora')
    sp.run(['git', 'commit', '-m', 'novizi rantigora a Cambi'])
